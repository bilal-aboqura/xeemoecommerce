  ...
  return (
    <section id="categories" className="py-20">
      <div className="mx-auto max-w-7xl px-5">
        ...
        >
          <div className="flex w-max snap-x snap-mandatory gap-4 lg:grid lg:w-auto lg:grid-cols-4 lg:gap-5">
            {cards.map((card) => (
              <Link
                key={card.href}
                href={card.href}
                className="group relative block h-[26rem] w-[min(76vw,19rem)] shrink-0 snap-start overflow-hidden rounded-lg border border-white/10 transition duration-300 hover:-translate-y-1 hover:border-brand/30 lg:h-[31rem] lg:w-auto"
              >
                <Image
                  src={card.image}
                  alt={card.title}
